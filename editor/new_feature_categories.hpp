...
public:
  ...
  vector<pair<string, uint32_t>> Search(string const & query, string const & lang) const;

  // Returns all registered names of categories in language |lang| and
  // types corresponding to these names.
  // The returned list is sorted.
  vector<pair<string, uint32_t>> GetAllCategoryNames(string const & lang);

private:
  indexer::CategoriesIndex m_index;
  vector<uint32_t> m_types;
  map<string, vector<pair<string, uint32_t>>> m_categoriesByLang;
