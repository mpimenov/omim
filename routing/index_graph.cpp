                                    ...
                                    vector<SegmentEdge> & edges)
{
  ...
  // todo(@m) Remove.
  if (m_roadAccess.IsBlocked(from.GetFeatureId()))
  {
    LOG(LINFO, ("moving through a barrier!"));
  }
  if (m_roadAccess.IsBlocked(to.GetFeatureId()))
  {
    // todo(@m) Remove.
    LOG(LINFO, ("cannot move due to a barrier"));
    return;
  }
